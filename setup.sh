...
replace "Mordovorot"        "$PROJECT_NAME"
replace "A console prototype of a sliding-row/column puzzle board game" "$PROJECT_DESCRIPTION"
replace "Kotlin"               "$STACK"
replace "MVVM (Model-View-ViewModel): board_model + viewmodel + view packages" "$ARCHITECTURE_SUMMARY"
replace "Kotlin official style guide"          "$CODE_STYLE"
replace "GitHub Issues"             "$PM_TOOL"
replace "GH"        "$ISSUE_PREFIX"
replace "https://github.com/lutsenko-yuriy/mordovorot/issues"      "$PM_PROJECT_URL"
replace "N/A (no experiment tracking)"     "$EXPERIMENT_TOOL"
replace "GitHub"            "$GIT_HOST"
replace "claude-opus-5, claude-sonnet-5, claude-haiku-4-5"   "$AVAILABLE_MODELS"
replace "Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>"  "$AI_COMMIT_TRAILER"
replace "🤖 Generated with Claude Code"     "$AI_TOOL_CREDIT"
