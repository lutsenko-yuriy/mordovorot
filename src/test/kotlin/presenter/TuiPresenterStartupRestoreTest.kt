 ...
 * [BasePresenter.offerStartupRestore], so this file only needs the idempotence guard plus one
 * cross-check that the TUI entry point runs the same prompt/track sequence, to guard against the
 * two entry points drifting (split GH-23 WU2).
 */
class TuiPresenterStartupRestoreTest {
    ...
    @Test
    fun `restoreOnStartup runs the same prompt-restore sequence as the console entry point`() {
        val board = FakeBoardModel().apply { correct = true }
