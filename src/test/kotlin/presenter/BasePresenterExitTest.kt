...
class BasePresenterExitTest {
    ...
    @Test
    fun `exit, save declined - ends play without saving`() = runTestBlocking {
        val board = FakeBoardModel()
        val saves = FakeSaveRepository()
        val analytics = RecordingAnalyticsService()
        val view = FakeView(confirmSaveBeforeExitResponses = mutableListOf(false))
        val presenter = TestPresenter(view, board, saves, analytics)

        assertFailsWith<ExitRequestedException> { presenter.exitGame() }
    ...
    @Test
    fun `exit, save confirmed with a name - saves then ends play`() = runTestBlocking {
        val board = FakeBoardModel()
        val saves = FakeSaveRepository()
        val analytics = RecordingAnalyticsService()
        val view = FakeView(
    ...
    @Test
    fun `exit, save confirmed but the save itself fails - returns normally and says so, without quitting`() = runTestBlocking {
    ...
    @Test
    fun `exit, a name with spaces re-prompts instead of discarding the save request, and a valid retry saves`() = runTestBlocking {
    ...
    @Test
    fun `exit, a name with spaces re-prompted then left blank - declines without saving`() = runTestBlocking {
        val board = FakeBoardModel()
        val saves = FakeSaveRepository()
        val analytics = RecordingAnalyticsService()
        val view = FakeView(
    ...
    @Test
    fun `exit, a name with a path separator re-prompts rather than aborting the quit as a save failure`() = runTestBlocking {
    ...
    @Test
    fun `exit, save confirmed but a blank or EOF name - ends play without saving`() = runTestBlocking {
        val board = FakeBoardModel()
        val saves = FakeSaveRepository()
        val analytics = RecordingAnalyticsService()
        val view = FakeView(
