...
class ConsolePresenterStartupRestoreTest {
    ...
    @Test
    fun `no saves - starts a new game immediately, no prompt`() = runTestBlocking {
    ...
    @Test
    fun `exactly one save, user confirms - restores it`() = runTestBlocking {
        val board = FakeBoardModel().apply { correct = true }
    ...
    @Test
    fun `exactly one save, user declines - starts a new game`() = runTestBlocking {
        val board = FakeBoardModel().apply { correct = true }
    ...
    @Test
    fun `two or more saves, exact name typed - restores it`() = runTestBlocking {
        val board = FakeBoardModel().apply { correct = true }
    ...
    @Test
    fun `two or more saves, blank input - starts a new game`() = runTestBlocking {
        val board = FakeBoardModel().apply { correct = true }
        val saves = FakeSaveRepository(
            mutableMapOf(
                "foo" to SavedBoard(4, fourByFour),
    ...
    @Test
    fun `two or more saves, unknown name re-prompts instead of falling back`() = runTestBlocking {
        val board = FakeBoardModel().apply { correct = true }
        val barState = intArrayOf(3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12)
        val saves = FakeSaveRepository(
            mutableMapOf(
    ...
    @Test
    fun `EOF at the startup prompt starts a new game, same as a decline - real EOF translation is covered in ViewImplCommandTest`() = runTestBlocking {
    ...
    @Test
    fun `a save picked at startup that fails to load is reported as new_game, not restored`() = runTestBlocking {
    ...
    @Test
    fun `an unknown name typed with two or more saves shows a message before re-prompting`() = runTestBlocking {
        val board = FakeBoardModel().apply { correct = true }
        val saves = FakeSaveRepository(
            mutableMapOf(
                "foo" to SavedBoard(4, fourByFour),
