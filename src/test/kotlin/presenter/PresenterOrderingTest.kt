 ...
 * "Why ordering is preserved"): [presenter.Presenter.uiRequests] is a rendezvous channel, and
 * `ask()` doesn't return until the View has actually answered - so a message the presenter emits
...
class PresenterOrderingTest {
    ...
    fun `a message emitted before a prompt is observed before the prompt request arrives`(): Unit = runBlocking {
        ...
        coroutineScope {
            val handler = launch {
                for (request in presenter.uiRequests) {
                    when (request) {
