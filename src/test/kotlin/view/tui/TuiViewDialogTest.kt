 ...
 * Driven via `FakeTerminal` + `RecordingAnalyticsService`, and either `FakeTuiPresenter` (board
 ...
 * through the presenter calling back into `TuiView`'s own `View` methods - a `FakeTuiPresenter`
 * doesn't do that).
 */
class TuiViewDialogTest {
    ...
    private fun view(terminal: FakeTerminal, presenter: FakeTuiPresenter, analytics: RecordingAnalyticsService = RecordingAnalyticsService()): TuiView =
    ...
    @Test
    fun `Save dialog happy path - typed name with no conflict saves and closes`() = runTestBlocking {
    ...
    @Test
    fun `a toolbar Save with a whitespace name is rejected instead of saved - console load could never split it back out`() = runTestBlocking {
    ...
    @Test
    fun `Save dialog shows the overwrite warning when the typed name matches an existing save`() = runTestBlocking {
        val presenter = FakeTuiPresenter()
    ...
    @Test
    fun `Save dialog Cancel closes without saving and tracks dialog_cancelled`() = runTestBlocking {
        val presenter = FakeTuiPresenter()
    ...
    @Test
    fun `Load dialog lists existing saves and Load on a selected row restores the board`() = runTestBlocking {
        val presenter = FakeTuiPresenter()
    ...
    @Test
    fun `Load dialog with no saves shows No saves found and Load is inert`() = runTestBlocking {
        val presenter = FakeTuiPresenter()
    ...
    @Test
    fun `Load dialog Cancel closes without loading and tracks dialog_cancelled`() = runTestBlocking {
        val presenter = FakeTuiPresenter()
    ...
    @Test
    fun `Exit dialog Yes opens the Save dialog then quits after a successful save`() = runTestBlocking {
    ...
    @Test
    fun `Exit dialog No quits without saving`() = runTestBlocking {
        val saves = FakeSaveRepository()
        val board = FakeBoardModel()
        val (exitX, exitY) = BoardLayout(terminalSize, board.SQUARE_SIDE, arrowsEnabled = true, modeButtons = listOf(InputMode.KEYBOARD, InputMode.CONSOLE)).exitButtonPosition()
        val exitDialog = Dialog(
    ...
    @Test
    fun `Exit dialog Cancel closes the dialog and exitGame is never called`() = runTestBlocking {
        val presenter = FakeTuiPresenter()
        val analytics = RecordingAnalyticsService()
    ...
    @Test
    fun `startup restore dialog appears when saves exist and Cancel starts a new game`() = runTestBlocking {
    ...
    @Test
    fun `screen views fire with the right opened_from for each dialog entry point`() = runTestBlocking {
        val presenter = FakeTuiPresenter()
        val analytics = RecordingAnalyticsService()
        val (saveX, saveY) = boardLayout(presenter).saveButtonPosition()
        val (loadX, loadY) = boardLayout(presenter).loadButtonPosition()
    ...
    @Test
    fun `a successful toolbar Save shows the presenter's confirmation message on the board afterward`() = runTestBlocking {
        val saves = FakeSaveRepository()
        val board = FakeBoardModel()
    ...
    @Test
    fun `a failed save during Exit Yes shows why the quit was aborted, and the session keeps running`() = runTestBlocking {
        val saves = FakeSaveRepository()
    ...
    @Test
    fun `a board status message does not leak into a dialog opened afterward`() = runTestBlocking {
        val saves = FakeSaveRepository(mutableMapOf("foo" to storage.SavedBoard(4, IntArray(16) { it })))
        val board = FakeBoardModel()
    ...
    @Test
    fun `confirmRestore renders the presenter-provided save name even if listSaves would disagree`() = runTestBlocking {
        val presenter = FakeTuiPresenter()
    ...
    @Test
    fun `clicking Save with an empty name cancels, same as pressing Enter on an empty name`() = runTestBlocking {
        val presenter = FakeTuiPresenter()
        val (saveX, saveY) = boardLayout(presenter).saveButtonPosition()
        val dialog = Dialog(Dialog.Kind.SAVE, "Save game", buttons = listOf(DialogButtonSpec("save", "Save"), DialogButtonSpec("cancel", "Cancel")))
        val saveButton = DialogLayout(dialog, terminalSize).buttons().first { it.target == HitTarget.DialogButton("save") }
    ...
    @Test
    fun `the exit flow's invalid-name explanation stays visible past the user's first corrective keystroke`() = runTestBlocking {
        val saves = FakeSaveRepository()
        val board = FakeBoardModel()
        val (exitX, exitY) = BoardLayout(terminalSize, board.SQUARE_SIDE, arrowsEnabled = true, modeButtons = listOf(InputMode.KEYBOARD, InputMode.CONSOLE)).exitButtonPosition()
        val exitDialog = Dialog(
    ...
    @Test
    fun `dialog content is truncated to the box width instead of overflowing the border on a narrow terminal`() = runTestBlocking {
