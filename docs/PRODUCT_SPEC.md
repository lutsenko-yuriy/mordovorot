...
- **Board navigation:** arrow keys move a highlighted cursor around the ring of shift arrows
  (`◀`/`▶`/`▲`/`▼`), shown in yellow; Enter or Space activates whichever arrow is highlighted,
  same effect as clicking it. Once solved, the cursor disappears and arrow keys/Enter go inert,
  matching the mouse mode's dimmed-arrows behavior.
- **Toolbar shortcuts**, shown in light blue in the controls hint and always live regardless of
  cursor position or solved state: **F5** opens Save, **F6** opens Load, **Escape** opens the Exit
  dialog (replacing F7 from earlier iterations of this ticket).
...
- All other dialog and startup-restore behavior (validation, re-prompting, analytics) is
  identical to Feature 6 — only the input method differs.
