...
3. **If this is the final WU** (the one that completes the ticket): invoke `debrief` now (`FEATURE.md` step 12), before shipping — the same order as the single-WU flow. **If this is an intermediate WU**: skip debrief; it runs exactly once, at the final WU.
   - **The final WU's approval order inverts the pattern from every prior WU.** WU1 through N-1 merge immediately on user approval (no debrief involved). The final WU does the opposite: on approval, invoke `debrief` first — merging happens later, as part of `ship` (step 4 below). Do not `merge` directly after approval on the final WU just because that was the right move on every WU before it (GH-42's debrief).
