...
> "Let's debrief GH-XX. How did it go overall?"
...
Append a dated block to the `## Debrief summary` section of `docs/knowledge/notes/GH-XX.md`.
...
```bash
git fetch origin
git checkout -b feature/GH-XX-debrief origin/main
git add <files>
git commit -m "meta: debrief GH-XX — <short summary>"
```

PR title: `meta: debrief GH-XX — <short summary>`. Include the PR URL in the report (step 8).

### 8. Report back

Confirm: knowledge base updated at `docs/knowledge/notes/GH-XX.md`, list any workflow/skill files changed (or "no file changes").
